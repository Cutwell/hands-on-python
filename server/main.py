...
class RoomManager:
    ...
    def process_message(self, json_string: str, client_id: int, room_id: int):
        ...
        if room_id in self.rooms:
            if message["type"] == "code":
                self.rooms[room_id].loc[client_id] = {"ready": True, "code": message["value"]}
    ...
    def run_loc_check(self, room_id: int):
        ...
        if self.rooms[room_id].ready:
            ...
            loc = "".join([user_data["code"] for user_id, user_data in self.rooms[room_id].loc.items()])
            messages.append(loc)
...
async def websocket_endpoint(websocket: WebSocket, client_id: int, room_id: int):
    try:
        await room_manager.connect_user(websocket=websocket, client_id=client_id, room_id=room_id)
        
        # broadcast an initial message with the current room participants
        await room_manager.broadcast(room_id=room_id, response={
            'terminal': [],
            'loc': room_manager.rooms[room_id].loc
        })

        while True:
            json_string = await websocket.receive_text()

            room_manager.process_message(json_string=json_string, client_id=client_id, room_id=room_id)
